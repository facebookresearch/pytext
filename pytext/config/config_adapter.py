...
@register_down_grade_adapter(from_version=39)
def v39_to_v38(json_config):
    for v in get_json_config_iterator(json_config, "vocab"):
        if "additional_tokens" in v:
            del v["additional_tokens"]
    return json_config


@register_adapter(from_version=38)
def v38_to_v39(json_config):
    # New config field was added with backwards-compatible default value
    return json_config
...
@register_down_grade_adapter(from_version=41)
def v41_to_v40(json_config):
    for v in get_json_config_iterator(json_config, "encoder"):
        if "uniform_quantization" in v:
            del v["uniform_quantization"]
    return json_config
...
@register_down_grade_adapter(from_version=44)
def v44_to_v43(json_config):
    for v in get_json_config_iterator(json_config, "Trainer"):
        if "discriminative_lr" in v:
            del v["discriminative_lr"]
        if "discriminative_lr_params_pattern" in v:
            del v["discriminative_lr_params_pattern"]
        if "freeze_params_pattern" in v:
            del v["freeze_params_pattern"]
    return json_config


@register_adapter(from_version=43)
def v43_to_v44(json_config):
    # New config field was added with backwards-compatible default value
    return json_config
