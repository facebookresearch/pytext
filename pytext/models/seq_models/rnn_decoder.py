...
from pytext.models.seq_models.base import PyTextDecoderBase
...
class DecoderWithLinearOutputProjection(PyTextDecoderBase):
    """
    Common super class for decoder networks with output projection layers.

    This class couples common functionality for PyText decoders with large
...
class RNNDecoderBase(PyTextIncrementalDecoderComponent):
    ...
    def __init__(
        ...
        self.use_additional_fc: bool = self.combined_output_and_context_dim != out_embed_dim
        if self.use_additional_fc:
            self.additional_fc = nn.Linear(
                self.combined_output_and_context_dim, out_embed_dim
            )
        else:
            # Create a dummy linear layer to appease Torchscript
            self.additional_fc = nn.Linear(1, 1)
    ...
    def _init_prev_states(
        ...
        self.set_incremental_state(
            incremental_state,
            "cached_feeds",
            self.initial_attn_context.expand(batch_size, self.attention.context_dim),
        )
