...
class BeamSearch_UTTERANCE_EXP(nn.Module):
    def __init__(
        self,
        model_list,
        tgt_dict_eos,
    ...
    def forward(
        self, src_tokens: Tuple[torch.Tensor], src_lengths: torch.Tensor, num_steps: int
    ):
        ...
        decoder_ip = self.encoder_ens(src_tokens, src_lengths)
        ...
        if self.record_attention:
            ...
            all_attn_weights = torch.zeros(
                num_steps + 1, self.beam_size, src_tokens[0].size(1)
            )
