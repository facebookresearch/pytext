...
class RNNGParserOp : public Operator<Context> {
 public:
  ...
  RNNGParserOp(const OperatorDef& operator_def, Workspace* ws)
    ...
    for (size_t i = 0; i < weightNames.size(); ++i) {
      stateMap.insert(
          {weightNames[i], autograd::Var(loadInput(i + RNNG_INPUTS_COUNT))}
      );
    } // Ignore RNNG_INPUTS

    parser_ = facebook::assistant::rnng::Parser(
  ...
  bool RunOnDevice() override {
    ...
    auto input_dict_weights =
        GetRepeatedInput<float>(RNNG_DICT_WEIGHTS);
    auto input_dict_lens =
        GetRepeatedInput<long>(RNNG_DICT_LENS);
 ...
 protected:
  ...
  // TODO: (jbai) T28169876 @jbai will move methods to public

  at::ScalarType atScalarTypeFor(const TypeMeta& meta) {
#define DEFINE_IF_2(ctype, aten_name, _) \
  if (meta.Match<ctype>()) {             \
    return at::k##aten_name;             \
  }
    AT_FORALL_SCALAR_TYPES(DEFINE_IF_2)
#undef DEFINE_IF_2
...
OPERATOR_SCHEMA(RNNGParser)
    ...
    .Output(
        RNNG_TOKENS,
        "tokens",
        "Tokens corresponding to actions")
    .Output(
        RNNG_SCORES,
        "scores",
        "Confidence scores corresponding to actions")
    .Output(
        RNNG_PRETTY_PRINT,
        "pretty_print",
        "Prediction in seqlogical form")
    .Arg(
        "model_config",
